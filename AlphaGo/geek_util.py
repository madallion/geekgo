...
def _sgf_init_gamestate(sgf_root):
	...
	gs = geekgo.GameState(int(s_size))
	# handle 'add black' property
	if 'AB' in props:
		for stone in props['AB']:
			gs.do_move(_parse_sgf_move(stone), geekgo.BLACK)
	# handle 'add white' property
	if 'AW' in props:
		for stone in props['AW']:
			gs.do_move(_parse_sgf_move(stone), geekgo.WHITE)
	# setup done; set player according to 'PL' property
	gs.current_player = geekgo.BLACK if s_player == 'B' else geekgo.WHITE
	return gs
